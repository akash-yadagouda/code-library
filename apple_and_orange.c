...
int t_apple=0,t_orange=0;
int min_dis_apple=s-a;
int min_dis_orange=t-b;
int range=t-s;
int max_range=range+min_dis_apple;
range=-range;
int max_range1=range+min_dis_orange;
for(int i=0;i<apples_count;i++)
{
    if(apples[i]>=min_dis_apple && apples[i]<=max_range )
    {
        t_apple++;
    }
...
{   
         ...
         if(oranges[i]<=min_dis_orange && oranges[i]>=max_range1 )
    {
        t_orange++;
    }
